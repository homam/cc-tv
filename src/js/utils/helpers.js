...
export class AnalyticsHelpers {
    ...
    static getTopCountries(metric = 'sales', limit = 5) {
        return Object.entries(state.countries)
            .map(([name, data]) => ({
                name,
                value: data.metrics?.[metric]?.current || data[metric] || 0,
                trend: data.metrics?.[metric]?.trend || 0,
                lat: data.lat,
                lon: data.lon
            }))
    ...
    static getCountryMetrics(countryKey) {
    ...
    static getStreamingEvents(limit = 10) {
    ...
    static getHourlyData(metric, hours = 24) {
    ...
    static getCountryPerformance(countryKey) {
        const country = state.countries[countryKey];
        if (!country || !country.metrics) return null;
    ...
    static getTrendIndicator(trend) {
        if (trend > 0) return '↗️';
        if (trend < 0) return '↘️';
        return '→';
    }
